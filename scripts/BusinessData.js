...
const NYbusiness = getBusiness();
export const newyorkCompaniesfiltered = NYbusiness.filter(businessNYObject => {
    if (businessNYObject.addressStateCode === "NY") {
        return true
    }
    return false
})
...
const manufacturingBusiness = getBusiness();
export const manufacturingFiltered = manufacturingBusiness.filter(manufacNYObject => {
    if (manufacNYObject.companyIndustry === "Manufacturing") {
        return true
    }
    return false
})

const purchaseBusiness = getBusiness();
export const purchasingAgents = purchaseBusiness.map(purchaseObject => {
    return {
        agentName: `${purchaseObject.purchasingAgent.nameFirst} ${purchaseObject.purchasingAgent.nameLast}`,
        company: purchaseObject.companyName,
        phoneNumber: purchaseObject.phoneWork
    }
})